...
describe("Initialize state", () => {
  test("shows empty", async () => {
    const { container } = render(<PostResultView
        postStatus={{ type: 'Initialize' }}
        services={[]}
    />);
...
describe("Process state", () => {
  test("should show service icon", async () => {
    render(<PostResultView
        postStatus={{ type: 'Process', results: [{service: 'Twitter', type: 'Posting'}]}}
        services={[]}
    />);
    screen.debug()
...
describe("Output state", () => {
  test("should show service icon and success message", async () => {
    render(<PostResultView
        postStatus={{ type: 'Output', results: [{service: 'Twitter', type: 'Success', url: 'https://twitter.com/'}]}}
        services={[]}
    />);
    screen.debug()
    expect(screen.getByAltText("Twitter")).toBeInTheDocument()
